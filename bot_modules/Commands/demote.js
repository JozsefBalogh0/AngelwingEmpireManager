...
module.exports = {
  run: async (client, msg, message, allCommands, user, member) => {
    ...
    if (user == undefined){
      ...
      .setDescription('Please provide who you want me to demote in the form of a mention.')
      .setFooter(`Example, !demote @Example_User#0000`)
    ...
    .setColor("#bb018a")
    .setTitle(`${message.author.username}, please specify what rank do you want me to demote ${member.username}#${member.discriminator} to`)
    .setDescription("Available demotes are:")
    ...
    //If user is already at lowest rank
    const lowerLimit = [guestRoleID, accessIRoleID]
    if (areArraysEqual(lowerLimit, userRoles)) {
      let responseEmbed = new Discord.MessageEmbed()
      .setColor('#FF0000')
      .setTitle(`I'm sorry, ${message.author.username}. I'm afraid I can't do that.`)
      .setDescription(`${member.username}#${member.discriminator} is already at the lowest rank I can give them.`)
    ...
    IDArray = [...AllIDs];
    IDArray.splice(userRank, IDArray.length)
    NameArray.splice(userRank, NameArray.length)

    const filtered = IDArray.filter((a) => a.length);

    const emojiArray = ["1️⃣", "2️⃣", "3️⃣", "4️⃣", "5️⃣", "6️⃣", "7️⃣", "8️⃣", "9️⃣", "🔟"]

    //Add fields for each possible demote
    filtered.forEach((item, i)=> {
      mainEmbed.addField(`${NameArray[i]}:`, `Press '${emojiArray[i]}' to demote **${member.username}#${member.discriminator}** to **${NameArray[i]}** rank.`)
    });
    ...
    await targetChannel.send(mainEmbed).then(async (main) => {
      ...
      await main.awaitReactions((reaction, user) => user.id == message.author.id && (emojiArray.includes(reaction.emoji.name)), { max: 1, time: 30000 }).then(async (collected) => {
        ...
        IDArray[indexOfID].forEach(async (item) => {
          tempArr2.push(item);
        ...
        let responseEmbed = new Discord.MessageEmbed()
        .setColor("#00FF00")
        .setTitle(`${message.author.username}, I've demoted ${member.username}#${member.discriminator} to ${NameArray[indexOfID]}.`)
        ...
        if (err.message == "Cannot read properties of undefined (reading 'emoji')"){
          let responseEmbed = new Discord.MessageEmbed()
          .setColor("#FF0000")
          .setTitle(`${message.author.username}, I've cancelled the demote operation.`)
          .setFooter("*I'll delete this message in 15 seconds.")
          await targetChannel.send(responseEmbed).then(async (sent) => {
            await sent.delete({timeout:15000});
          });
          ...
          .setDescription(`Demote Operation Cancelled:\n*${err.name}*\n*${err.message}*\n\nMore info in console.`)
  ...
  help: {
    ...
    example: ("!demote @Example_User#0000"),
